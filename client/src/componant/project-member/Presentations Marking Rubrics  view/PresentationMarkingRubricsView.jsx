...
//       const filteredRubrics = response.data.filter(rubric => rubric.type === 'Proposal Presentation' || rubric.type === 'Progress 1 Presentation'|| rubric.type === 'Progress 1 Presentation'|| rubric.type === 'Progress 2 Presentation'|| rubric.type === 'Final Presentation');
...
//           .filter(rubric => rubric.type === 'Proposal Presentation' || rubric.type === 'Progress 1 Presentation'|| rubric.type === 'Progress 1 Presentation'|| rubric.type === 'Progress 2 Presentation'|| rubric.type === 'Final Presentation')
...
const PresentationMarkingRubricsView = () => {
  ...
  const fetchMarkingRubrics = async () => {
    try {
      ...
      const filteredRubrics = response.data.filter(rubric => 
        rubric.type === 'Proposal Presentation' ||
        rubric.type === 'Progress 1 Presentation' ||
        rubric.type === 'Progress 2 Presentation' ||
        rubric.type === 'Final Presentation'
      );
      setMarkingRubrics(filteredRubrics);
    } catch (error) {
      console.error('Error fetching marking rubrics:', error);
  ...
  const handleDeleteAll = async () => {
    try {
      ...
      if (result.isConfirmed) {
        const rubricIdsToDelete = markingRubrics.map(rubric => rubric._id);
